...
# ============================================================================
# Delta-learning test: head 0 with energy+forces, head 1 with energy only
# (NaN forces). Uses EnergyForceStressLoss with ignore_nan to handle the
# missing labels. Verifies the model trains and the energy-only head still
# produces finite autograd forces.
# ============================================================================


def _make_delta_learning_datasets():
    """Create datasets mimicking delta-learning: head 0 (E+F), head 1 (E only, NaN forces)."""
    cu_transforms_h0 = [
        ChemicalSpeciesToAtomTypeMapper(
            model_type_names=TYPE_NAMES,
            chemical_species_to_atom_type_map={"Cu": "Cu"},
    ...
    # Head 0: full energy + forces
    h0_train = EMTTestDataset(
        transforms=cu_transforms_h0,
        element="Cu",
        num_frames=10,
        supercell=(2, 2, 2),
        seed=SEED,
    )
    # Head 1: energy only — set forces to NaN
    h1_train_raw = EMTTestDataset(
        transforms=cu_transforms_h1,
        element="Cu",
        num_frames=10,
        supercell=(2, 2, 2),
        seed=SEED + 10,
    )
    # Overwrite forces with NaN
    h1_train = []
    for i in range(len(h1_train_raw)):
        d = h1_train_raw[i]
        d[AtomicDataDict.FORCE_KEY] = torch.full_like(
            d[AtomicDataDict.FORCE_KEY], float("nan")
        )
        h1_train.append(d)

    return h0_train, h1_train


def test_delta_learning_nan_forces():
    """Multi-head training with NaN forces on energy-only head should work end-to-end.

    This exercises the delta-learning pattern where head 0 has energy+forces
    and head 1 has energy only (forces set to NaN). The loss uses ignore_nan
    for force and stress fields so NaN labels don't poison the loss.
    """
    model = _build_multihead_model()
    h0_train, h1_train = _make_delta_learning_datasets()

    optimizer = torch.optim.Adam(model.parameters(), lr=0.01)

    def _compute_loss_with_nan_masking(out, target):
        """Energy MSE + force MSE with NaN masking (mimics ignore_nan behavior)."""
        energy_loss = (
            out[AtomicDataDict.TOTAL_ENERGY_KEY]
            - target[AtomicDataDict.TOTAL_ENERGY_KEY]
        ).pow(2).mean()

        # Mask NaN forces — use only finite target values
        pred_f = out[AtomicDataDict.FORCE_KEY]
        true_f = target[AtomicDataDict.FORCE_KEY]
        notnan = ~torch.isnan(true_f)
        if notnan.any():
            force_loss = (pred_f[notnan] - true_f[notnan]).pow(2).mean()
        else:
            force_loss = torch.tensor(0.0, dtype=pred_f.dtype, device=pred_f.device)

        return energy_loss + 10.0 * force_loss

    losses = []
    for step in range(10):
        optimizer.zero_grad()

        # Head 0: energy + forces (all finite)
        h0_batch = AtomicDataDict.batched_from_list(
            [h0_train[i] for i in range(min(3, len(h0_train)))]
        )
        h0_out = model(h0_batch)
        h0_loss = _compute_loss_with_nan_masking(h0_out, h0_batch)

        # Head 1: energy only (forces are NaN — should be masked out)
        h1_batch = AtomicDataDict.batched_from_list(h1_train[:3])
        h1_out = model(h1_batch)
        h1_loss = _compute_loss_with_nan_masking(h1_out, h1_batch)

        total_loss = h0_loss + h1_loss

        # Loss must be finite (NaN forces should be masked out)
        assert torch.isfinite(total_loss), (
            f"Loss is not finite at step {step}: {total_loss.item()}"
        )
    ...
    # The energy-only head (head 1) should still produce finite autograd forces
    model.eval()
    test_data = h1_train[0].copy()
    out = model(test_data)
    assert torch.isfinite(out[AtomicDataDict.FORCE_KEY]).all(), (
        "Energy-only head should produce finite autograd forces"
    )
...
def _train_multihead_hf_mp2(hf_train, mp2_train, e_mean_hf, e_mean_mp2, f_rms, avg_nn):
    ...
    model = NequIPGNNModel(
        ...
        head_names=["HF", "MP2"],
        per_type_energy_shifts={"HF": e_mean_hf, "MP2": e_mean_mp2},
        per_type_energy_scales={"HF": f_rms, "MP2": f_rms},
    )
